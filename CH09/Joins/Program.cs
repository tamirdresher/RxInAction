...
namespace Joins
{
    class Program
    {
        ...
        private static void GroupJoin()
        {
            ...
            IObservable<DoorOpened> enterences = doorOpened.Where(o => o.Direction == OpenDirection.Entering);
            IObservable<DoorOpened> maleEntering = enterences.Where(x => x.Gender == Gender.Male);
            IObservable<DoorOpened> femaleEntering = enterences.Where(x => x.Gender == Gender.Female);

            IObservable<DoorOpened> exits = doorOpened.Where(o => o.Direction == OpenDirection.Leaving);
            IObservable<DoorOpened> maleExiting = exits.Where(x => x.Gender == Gender.Male);
            IObservable<DoorOpened> femaleExiting = exits.Where(x => x.Gender == Gender.Female);
