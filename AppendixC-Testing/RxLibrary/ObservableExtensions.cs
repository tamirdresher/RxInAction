...
namespace RxLibrary
{
    public static class ObservableExtensions
    {
        ...
        /// <summary>
        /// Emits the first value from every burst of items. a burst is a sequence of elements from
        /// an observable sequence which are followed by another element within a specified relative
        /// time duration.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="src">Source sequence to throttle.</param>
        /// <param name="maximalDistance">
        /// The due-time after an emission that defines the end of a burst.
        /// </param>
            ...
            IScheduler scheduler)
        {
            return src.Publish(xs => {
                IObservable<T> windowBoundaries = xs.Throttle(maximalDistance, scheduler);
        ...
        {
            return src.Publish(xs => {
                IObservable<T> maximDurationPassed = xs.Delay(maximalBurstDuration, scheduler).Take(1);
                IObservable<T> windowBoundary = xs.Throttle(maximalDistance, scheduler).Merge(maximDurationPassed);
